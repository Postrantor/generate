...
> 然后，该特性可以用于构造，如[`static_assert`](http://stackoverflow.com/documentation/c%2b%2b/3822/static-assert#t=2016072810077230765701）或[`std:：enable_if`](http://stackoverflow.com/documentation/c%2b%2b/1169/sfinae-如果#t=20160728100647141537，则替换失败不是错误/3777/启用）。带有“std:：is_pointer”的示例：
...
> “std:：is_same＜T，T＞”类型关系用于比较两种类型。它将计算为布尔值，如果类型相同则为true，否则为false。
...
> “std:：is_same”类型关系也将工作，而与typedef无关。在第一个例子中，当比较“int==int 32_t”时，这一点实际上得到了证明，但这并不完全清楚。
...
> 对所有整数类型“int”、“char”、“long”、“unsigned int”等求值为true。
...
> 对所有浮点类型计算为true`float、double、long double等。
...
> 除“enum class”外，所有类和结构的计算结果均为true。
